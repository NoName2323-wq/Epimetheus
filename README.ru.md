...
![Epimetheus](https://img.shields.io/badge/Проект-Epimetheus%20v2.5.0-blueviolet?style=for-the-badge)
...
![Тесты](https://img.shields.io/badge/Тесты-19%20Пройдено-brightgreen?style=for-the-badge)
...
> [!IMPORTANT]
> **Программа Epimetheus разработана исключительно для Linux.** Поддержка Windows была полностью удалена из исходного кода: удалены все устаревшие `.exe` обёртки, Windows DLL и зависимости в пользу максимальной производительности POSIX, нативной изоляции ELF-процессов и надёжного управления таймаутами. При попытке запуска на Windows скрипт выдаст понятное сообщение об ошибке и безопасно завершится.
...
* **Сурсы обфускатора Prometheus:** Изучен и исследован по официальному исходному коду [Prometheus Obfuscator от wcrddn / levno-710](https://github.com/wcrddn/Prometheus/tree/master).
* **Эволюция в Epimetheus:** Код был фундаментально переработан в высокопроизводительный Linux-движок с модульной AST-оптимизацией, потоковой фильтрацией логов VM и чистым статическим декодером на Python.
...
* **[`engine/ast_optimizer.py`](engine/ast_optimizer.py) (Оптимизатор AST и токенов Lua):**
  - **Свёртка константных выражений (Constant Folding):** Безопасно вычисляет обфусцированную арифметику Prometheus (например, `(-80732 + 80796)` $\rightarrow$ `64`, `(0x497e0 + -301009)` $\rightarrow$ `15`, шестнадцатеричные и десятичные числа с базовыми операторами) без риска выполнения вредоносного кода или деления на ноль.
...
* **[`engine/static_decoder.py`](engine/static_decoder.py) (Статический декодер констант):**
  ...
  - Распознаёт шаг циклического сдвига (`Rotate`: `{{1, LEN}, {1, SHIFT}, ...}`) и обращает сдвиг для восстановления оригинальных индексов.
...
* **Лексический нормализатор Luau:** Преобразует составные операторы присваивания (`+=`, `-=`, `*=`, `/=`, `%=`, `..=`) в синтаксис Lua 5.1 без повреждения бинарных строк.
...
```mermaid
flowchart TD
    A["Обфусцированный Luau / Lua скрипт"] --> B["deobfuscator.py: Лексический нормализатор"]
    ...
    J -->|Свёртка арифметики и алиасов| K[".deobf.lua (Чистый исходный код)"]
    C -->|Дамп массива памяти| L[".constants.lua (Таблица констант)"]
    E -.->|Статическая сверка| L
```
...
| Параметр | Исходный дампер | Движок Epimetheus (v2.5.0) | Прирост |
| :--- | :--- | :--- | :--- |
...
| **Набор тестов** | 9 тестов (базовые) | **19 тестов** (100% OK) | **Полное покрытие** |
