...
![Тесты](https://img.shields.io/badge/Тесты-30%20Пройдено-brightgreen?style=for-the-badge)
...
* **[`engine/syntax_normalizer.py`](engine/syntax_normalizer.py) (Нормализатор синтаксиса Luau):**
  ...
  - **Очистка аннотаций типов Luau:** Удаляет статические типы (`local x: number = 1` $\rightarrow$ `local x = 1`, `type Foo = ...`), устраняя синтаксические ошибки при разборе стандартным парсером Lua 5.1.
...
* **Надёжность глобального окружения:** Добавлен проброс через метатаблицу `MockEnv` для `_G` и `getfenv()`, исключающий ошибки `nil` при динамических обращениях обфускатора к сервисам Roblox.
...
| **Набор тестов** | 9 тестов (базовые) | **30 тестов** (100% OK) | **Полное покрытие** |
