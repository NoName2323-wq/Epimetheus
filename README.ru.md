...
![Epimetheus](https://img.shields.io/badge/Проект-Epimetheus%20v2.7.0-blueviolet?style=for-the-badge)
...
![Тесты](https://img.shields.io/badge/Тесты-54%20Модульных%20Теста-brightgreen?style=for-the-badge)
...
Результаты тестирования на реальных скриптах Прометея (пайплайн v2.7.0):

| Параметр | Исходный дампер | Движок Epimetheus (v2.7.0) | Прирост |
| :--- | :--- | :--- | :--- |
...
| **Скорость подстановки переменных** | ~1.61 сек на блок | **0.008 сек** на блок | **В 200 раз быстрее** |
| **Пропускная способность AST-вычислений** | ~80 000 выражений/с | **>18 000 000 выражений/с** | **В >200 раз быстрее** |
| **Распространение алиасов AST (5к строк)** | ~1.80 сек | **0.20 сек** | **В 9 раз быстрее** |
| **Нормализатор синтаксиса Luau** | ~650 КБ/с | **>2 600 КБ/с** | **В >4 раза быстрее** |
| **Дисковый I/O промежуточного пайплайна** | Повторные чтения с диска | **0 (чистый in-memory streaming)** | **Устранены лишние обращения к диску** |
| **Объём рабочей директории** | >26 МБ | **5.9 МБ** | **Экономия 77% диска** |
| **Набор тестов** | 9 тестов (базовые) | **54 модульных теста** (50 активных, 4 локальных) | **Комплексный набор тестов** |
...
### Пакетная деобфускация директории (последовательно или параллельно)
...
# Параллельная пакетная обработка на нескольких ядрах CPU:
python3 deobfuscator.py путь/к/папке_со_скриптами --jobs 4
# или автоматический подбор количества воркеров:
python3 deobfuscator.py путь/к/папке_со_скриптами -j auto
```
*(По умолчанию при запуске без аргументов проверяется папка `obfuscated_scripts/`)*

### Запуск бенчмарков производительности
Запуск многоэтапного замера производительности подсистем движка:
